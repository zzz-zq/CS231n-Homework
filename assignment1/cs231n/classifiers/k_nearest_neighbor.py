...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        # dists += np.dot((np.sum(X ** 2, axis=1)).T.reshape(500,1) , np.ones((1, 5000)))
        dists += (np.sum(X ** 2, axis=1)).T.reshape(500,1) * np.ones((1, 5000))
        dists += np.ones((500, 1)) * np.sum(self.X_train ** 2, axis=1).reshape(1,5000)
